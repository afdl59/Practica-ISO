...
function TiroLibre() {
  ...
  const handleScoreUpdate = async (newPoints) => {
    ...
    if (isAuthenticated) {
      try {
        const response = await fetch(`/api/users/${username}/score`, {
          ...
          body: JSON.stringify({ category: 'tiroLibre', newScore }),
        });
        if (!response.ok) throw new Error('Error al actualizar la puntuación');
        setMessage('¡Puntuación actualizada en el servidor!');
      } catch (error) {
        console.error('Error al guardar la puntuación:', error);
        setMessage('Error al actualizar la puntuación en el servidor.');
      }
    } else {
      setMessage(`¡Has obtenido ${newPoints} puntos! Inicia sesión para guardar tu puntuación.`);
    }
  };
  ...
  const shoot = () => {
    ...
    if (isGoal) {
      handleScoreUpdate(10); // Sumar 10 puntos por gol
      setMessage('¡Gol!');
    } else {
